...
object WorkoutCoachingRules {
    ...
    fun preIntervalAlert(ctx: RideContext): CoachingEvent? {
        ...
        val message = if (isFirst) {
            when (ws.workoutType) {
                WorkoutType.VO2_MAX -> "First rep. Go hard."
                WorkoutType.THRESHOLD -> "First block. Don't overcook."
                WorkoutType.RECOVERY_RIDE -> "Easy effort. Stay Z1."
                else -> "First block. Settle in."
            }
        } else {
     ...
     */
    fun preIntervalFueling(
        ctx: RideContext,
        fuelingThresholdGrams: Int = 10,
    ): CoachingEvent? {
        val ws = ctx.workout
        if (!ws.isActive) return null
        ...
        if (deficit < fuelingThresholdGrams) return null
        ...
        return CoachingEvent(
            ...
            message = "Eat now. ${deficit}g deficit.",
            priority = CoachingPriority.HIGH,
    ...
    fun powerAboveTarget(ctx: RideContext): CoachingEvent? {
        ...
        val message = when (ws.workoutType) {
            WorkoutType.THRESHOLD -> "Back off. ${overBy}W over ceiling."
            WorkoutType.SWEET_SPOT -> "Ease back. ${overBy}W too high."
            WorkoutType.VO2_MAX -> "Too high. Drop ${overBy}W."
            WorkoutType.OVER_UNDER -> "Drop ${overBy}W. Over phase done."
            WorkoutType.RECOVERY_RIDE -> "${overBy}W over Z1. Ease off."
            else -> "Ease back. ${overBy}W over target."
        }
    ...
    fun powerBelowTarget(ctx: RideContext): CoachingEvent? {
        ...
        val message = when (ws.workoutType) {
            WorkoutType.VO2_MAX -> "Dig in. ${belowBy}W below target."
            WorkoutType.THRESHOLD -> "More power. ${belowBy}W low."
            WorkoutType.SWEET_SPOT -> "Lift it. ${belowBy}W below."
            WorkoutType.ENDURANCE_SURGES -> "Surge — up ${belowBy}W. Go."
            WorkoutType.OVER_UNDER -> "Under phase. Hold ${belowBy}W low."
            else -> "Push. ${belowBy}W below target."
        }
    ...
    fun powerOnTarget(ctx: RideContext): CoachingEvent? {
        ...
        val message = when (ws.workoutType) {
            WorkoutType.SWEET_SPOT -> "Dialed in. Hold ${avg30s}W."
            WorkoutType.THRESHOLD -> "On threshold. Hold steady."
            WorkoutType.VO2_MAX -> "Good pace. Stay there."
            WorkoutType.OVER_UNDER -> "Over phase. Hold ${avg30s}W."
    ...
    fun cadenceDropping(ctx: RideContext, settingsMinCadence: Int = 75): CoachingEvent? {
        ...
        val message = when (ws.workoutType) {
            WorkoutType.VO2_MAX -> "Spin up. ${minCadence}+ rpm."
            WorkoutType.THRESHOLD -> "Shift lighter. Cadence low."
            WorkoutType.SWEET_SPOT -> "Cadence up. Shift lighter."
    ...
    fun hrCeilingExceeded(ctx: RideContext): CoachingEvent? {
        ...
        val message = when (ws.workoutType) {
            WorkoutType.RECOVERY_RIDE -> "+${overBy}bpm above Z1. Ease off."
            WorkoutType.ENDURANCE_SURGES -> "Post-surge HR high. Settle."
            else -> "HR +${overBy}bpm. Ease off."
        }
    ...
    fun hrBelowTarget(ctx: RideContext): CoachingEvent? {
        ...
        return CoachingEvent(
            ...
            message = "HR -${belowBy}bpm. Push more.",
            priority = CoachingPriority.MEDIUM,
            alertStyle = AlertStyle.COACHING,
    ...
    fun hrOnTarget(ctx: RideContext): CoachingEvent? {
        ...
        return CoachingEvent(
            ...
            message = "HR ${ctx.heartRateBpm}bpm. Hold it.",
            priority = CoachingPriority.LOW,
            alertStyle = AlertStyle.POSITIVE,
            suppressIfFiredInLastSec = policy.onTargetSuppressionSec,
        )
    ...
    fun recoveryNotRecovering(ctx: RideContext): CoachingEvent? {
        ...
        val message = when (ws.workoutType) {
            WorkoutType.VO2_MAX -> "Recover. Next rep needs you fresh."
            WorkoutType.THRESHOLD -> "Drop power. Threshold needs rest."
            else -> "Recover. Drop to Z1."
        }
    ...
    fun powerFadingTrend(ctx: RideContext): CoachingEvent? {
        ...
        val message = when (ws.workoutType) {
            WorkoutType.VO2_MAX -> "Power fading. Reassess after this."
    ...
    fun recoveryQualityDeclining(ctx: RideContext): CoachingEvent? {
        ...
        val message = when (ws.workoutType) {
            WorkoutType.VO2_MAX -> "Recovery slowing. Stop soon."
            WorkoutType.THRESHOLD -> "Recovery slowing. Cut or -5%."
            else -> "Recovery slowing. Reduce load."
        }
    ...
    fun sessionComplete(ctx: RideContext): CoachingEvent? {
        ...
        val msg = when {
            completedCount <= 0 -> "Done. Easy from here."
            ws.workoutType == WorkoutType.VO2_MAX ->
                "VO2max done. ${completedCount} reps. Fuel!"
            ws.workoutType == WorkoutType.THRESHOLD ->
                "Threshold done. Recover well."
            ws.workoutType == WorkoutType.SWEET_SPOT ->
                "Sweet spot done. Fuel up!"
            else -> "Session done. Fuel up!"
        }
    ...
    fun lastIntervalMotivation(ctx: RideContext): CoachingEvent? {
        ...
        val message = when {
            recoverySlow && ws.workoutType == WorkoutType.VO2_MAX ->
                "Last rep. Start easy."
            recoverySlow ->
                "Final block. Start steady."
            ws.workoutType == WorkoutType.VO2_MAX ->
                "Last rep. Go all in."
            ws.workoutType == WorkoutType.THRESHOLD ->
                "Final block. Hold to the end."
            ws.workoutType == WorkoutType.SWEET_SPOT ->
    ...
    fun evaluateAll(
        ctx: RideContext,
        settingsMinCadence: Int = 75,
        fuelingThresholdGrams: Int = 10,
    ): List<CoachingEvent> =
        listOfNotNull(
            preIntervalAlert(ctx),
            preIntervalFueling(ctx, fuelingThresholdGrams),
